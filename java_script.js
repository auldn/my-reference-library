...
function renderEntry(r) {
  ...
  const jEl = document.createElement('em');
  jEl.textContent = journal;
  meta.appendChild(jEl);

  meta.appendChild(document.createTextNode(' · '));
  meta.appendChild(document.createTextNode(String(year)));

  // ---- Controls row ----
  const controls = document.createElement('div');
  controls.className = 'row';

  const pmidWrap = document.createElement('div');
  ...
  copyBtn.textContent = '📋';
  if (!r.pmid) copyBtn.disabled = true;

  copyBtn.addEventListener('click', async () => {
    try {
      await navigator.clipboard.writeText(String(r.pmid));
      setStatus('PMID copied');
  ...
  pmidWrap.appendChild(label);
  pmidWrap.appendChild(codeEl);
  pmidWrap.appendChild(copyBtn);
  ...
  absContainer.className = 'abstract' + (r._abstractShown ? '' : ' hidden');
